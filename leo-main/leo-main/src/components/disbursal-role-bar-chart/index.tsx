...
const DisbursalRoleBarChart = ({
  ...
  const { roleDisbursalAmountData, isFetchingRoleDisbursalAmountData } =
    useRoleDisbursalAmount({ role, month, year });

  if (isFetchingRoleDisbursalAmountData) {
    return (
    ...
    <Card>
      <div className="flex flex-col">
        <div className="flex-col flex">
          ...
          <Subtitle className="my-4">{`Disbursals per ${enumCleaner(
            role,
      ...
      <BarChart
        ...
        index="name"
        categories={['Disbursal Amount', 'Disbursal Target']}
        colors={color}
        showAnimation={true}
...
export default DisbursalRoleBarChart;
...
export const CustomTooltip = ({
  ...
  const totalDisbursalPercent = Math.ceil(
  ...
  return (
    <div className="bg-white p-4 rounded-tremor-default shadow-tremor-dropdown border border-tremor-border text-tremor-content">
      ...
      <div className="flex flex-row items-center space-x-4 mt-2">
        <p className="text-tremor-content text-sm">Total Disbursed</p>
        <p className="font-medium tabular-nums text-right whitespace-nowrap text-tremor-content-emphasis text-sm">
          {isNaN(totalDisbursalPercent) ? 0 : totalDisbursalPercent}%
        </p>
      </div>
    </div>
  );
