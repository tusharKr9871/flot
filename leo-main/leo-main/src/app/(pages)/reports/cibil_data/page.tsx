...
import { useFetchCibilData } from '@/hooks/reports-api';
...
import CibilDataTable from '@/components/cibil-data-table';
import { handleDownload } from '@/utils/utils';

export type CibilDataDownloadType = {
  id: string;
  ['Name']: string;
  ['DOB']: Date;
  ['Gender']: string;
...
const CibilData = () => {
  ...
  const { cibilData, isFetchingCibilData } = useFetchCibilData({
    pageNumber,
    searchTerm,
  ...
  const fetchData = async () => {
    try {
      ...
      const url = new URL(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/download-reports/download-cibil-data`,
      );
      ...
      handleDownload(response.data, 'cibil-data');
      setProgress(100);
      setIsOpen(false);
      setProgress(0);
      setLoading(false);
      ...
      <div className="md:px-14 mx-2 pt-4 pb-4">
        <Card className="h-auto flex flex-col">
                ...
                {(user?.role === 'Admin' ||
                  ...
                  <PrimaryCTA
                    ...
                    onClick={() => {
                      ...
                      fetchData();
                      setProgressFunction({
                        totalTimeInSeconds: (cibilData?.count || 1) / 35,
                      });
                    }}
                    icon="export"
                    loading={loading}
          ...
          {isFetchingCibilData ? (
            ...
            <CibilDataTable
              tableData={cibilData?.cibilData || []}
              total={cibilData?.count || 1}
